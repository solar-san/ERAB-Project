---
title: "Gender and Risk Aversion"
author: "Marco Solari, 875475"
subtitle: "The role of gender on risk-taking: analysing the Bank of Italy IACOFI dataset"
output:
  pdf_document:
    toc: true
...
```{r reading data setup}
read_data <- function(
    filename, 
    directory = "/Users/themagician/Documents/DABS/Appunti/Economics_of_Risk_and_Agent's_Behaviour/Project/Data/"
    ) {
  filepath <- paste0(directory, 
                     filename
                     )
  read.csv(filepath,
           header = T
...
```{r create dataframe}
iacofi_full <- read_data(
  filename = 'iacofi2020.csv'
) %>%
  select(-anno)
```
...
```{r household head plot, fig.width=6, fig.height=2, fig.cap='The inequality is overwhelming; in particular, the proportion of males declaring that their wife is earning a larger income in the family is close to being invisible.'}
iacofi_data %>% 
  ggplot(
    aes(
      fill = gender,
  ...
  theme(
    ...
    axis.text.y = element_text(
      #angle = 50, vjust = 1, hjust = 1, size = 12
      )
    )
```
...
```{r education distribution, fig.width=6, fig.height=3, fig.cap='Number of persons by education level and geographic area in Italy'}
iacofi_data %>%
  filter(!is.na(edu)) %>% 
...
```{r distribution of income, fig.width=12, fig.height=12, message=FALSE, warning=FALSE, fig.cap='Densities of income (x-axis) by degree (grouping variable on the y-axis) and area (each density curve representing an Italian region). Living in the northern regions and having spent more years pursuing a degree consistently relates to higher income and less variability.'}
iacofi_data %>%
  filter(
    !is.na(
      edu
...
$$
\rho_i = \alpha + \beta \times \mathrm{gender}_i + {\Theta} \times \mathbf{X}_i + \epsilon_i
$$
